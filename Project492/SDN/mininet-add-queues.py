...
switches = []
switches.append('s1') #Implement rule only in switch 1
#for bn in brdgs:
 #       sw =  p[(bn+8):(bn+10)]
#	print sw;
        #switches.append(sw)
...
for sw in switches:
        queuecmd = "sudo ovs-vsctl %s -- --id=@defaultqos create qos type=linux-htb other-config:max-rate=1000000000 queues=0=@q0,1=@q1,2=@q2,3=@q3 -- --id=@q0 create queue other-config:min-rate=1000000000 other-config:max-rate=1000000000 -- --id=@q1 create queue other-config:max-rate=2000000 -- --id=@q2 create queue other-config:max-rate=20000000 -- --id=@q3 create queue other-config:max-rate=200000000 other-config:min-rate=2000000" % config_strings[sw]
