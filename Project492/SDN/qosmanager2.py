...
def write_add(op,json_o=None):
    ...
    sjson = sjson.replace('} "', '}')
    newd.write(sjson)
    state = os.popen("echo '%s' | python3 -mjson.tool | more" % sjson).read()
    #print state #debug
    newd.close()
...
def listServices(ip,port):
	...
	try:
	 ...
	 srvs = os.popen("echo '%s' | python3 -mjson.tool | more" % req).read()
...
def listPolicies(ip,port):
	...
	try:
	 ...
	 pols = os.popen("echo '%s' | python3 -mjson.tool | more" % req).read()
